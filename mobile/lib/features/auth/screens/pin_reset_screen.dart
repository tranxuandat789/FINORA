...
class _PinResetScreenState extends State<PinResetScreen> {
  ...
  void _resetPin(String newPin) {
    context.read<AuthProvider>().resetPin(
      widget.email,
      widget.resetToken,
      newPin,
      onSuccess: () {
        SnackBarUtils.showTopSnackBar(context, 'Khôi phục mã PIN thành công', isSuccess: true);
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                _isConfirming ? 'Xác nhận mã PIN mới' : 'Nhập mã PIN mới',
                style: GoogleFonts.inter(
                  fontSize: 24,
